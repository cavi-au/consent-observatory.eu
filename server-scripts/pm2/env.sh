...
export MAIL_SMTP_USER="" # SMTP-server username (most likely the email address)
export MAIL_SMTP_PASS="" # SMTP-server password (most likely password for the email address)
export MAIL_MESSAGE_FROM="" # [REQUIRED] the email to set as the `from` field (e.g. `info@example.com` or `"John Doe" <info@example.com>`)
export ADMIN_ACCESS_SECRET="" # [REQUIRED] a secret code for logging into the admin panel must be >= 8 characters long
